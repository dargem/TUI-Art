...
TEST(TerminalStatus, StatusAcceptsAndRemovesSubscribers) {
...

    {
        ...
        EXPECT_TRUE(terminalStatus.getNumberListeners() == (startingListeners + 1))
            << "Terminal status should gain exactly one listener after adding just one";

    }  // token gets destroyed, unsubscribing it fom listeners
    EXPECT_TRUE(terminalStatus.getNumberListeners() == startingListeners)
        << "Terminal status should have original number of listeners after deleting access token";
}
...
TEST(TerminalStatus, StatusUpdatesOnlySubscribers) {
    ...
    {
        TerminalDimensionToken token{terminalStatus.addDimensionListener(&listener)};
        EXPECT_FALSE(listener.terminalDimension.has_value())
            << "Listener should start with no value";
    ...
    EXPECT_FALSE(listener.terminalDimension.has_value())
        << "Listener shouldn't be updated after unsubscribing";
}
