...
// credits to https://github.com/johnBuffer/StableIndexVector

namespace utils {
/** Alias the differentiate between IDs and index.
 * An ID allows to access the data through the index vector and is associated with the same object
...
/** A standalone struct allowing to access an object without the need to have a reference to the
 * containing SparseSet.
 *
...
class Handle {
   public:
    ...
    Handle() = default;
    /// Constructor
    Handle(ID id, ID validity_id, SparseSet<TObjectType>* vector) :
            m_id{id}, m_validity_id{validity_id}, m_vector{vector} {}
     ...
     *
     * @return false if uninitialized or if the object has been erased from the vector, true
     * otherwise
     */
   ...
   private:
    ...
    ID m_id = 0;
    /// The validity ID of the object at the time of creation. Used to check the validity of the
    /// handle.
    ID m_validity_id = 0;
    /// A raw pointer to the vector containing the object associated with this handle
    SparseSet<TObjectType>* m_vector = nullptr;

    /// Used to perform debug checks
    friend class SparseSet<TObjectType>;
};

/** A vector that provide stable IDs when adding objects.
 * These IDs will still allow to access their associated objects even after inserting of removing
 * other objects. This comes at the cost of a small overhead because of an additional indirection.
 *
...
class SparseSet {
   ...
   private:
    ...
    struct Metadata {
        ...
        ID rid = 0;
        /// An identifier that is changed when the object is erased, used to ensure a handle is
        /// still valid.
        ID validity_id = 0;
    };
    ...
    /// The vector holding the associated metadata. It is accessed using the same index as for the
    /// data vector.
