    ...
    public static BranchParams fromFile(String path)
    {
        ...
        try (InputStream in = BranchParams.class.getResourceAsStream(path)) 
        {
            if (in != null) 
            {
                ...
                double death_chance = Double.parseDouble(p.getProperty("death_chance"));
                ...
                return new BranchParams(initial_length, length_decay, width_decay, death_chance, angle_variance, branches_per_node);
            }
            else
            {
